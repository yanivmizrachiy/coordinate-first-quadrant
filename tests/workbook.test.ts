...
describe('workbook integrity (USER_MEMORY.md mandatory checks)', () => {
  it('has exactly 56 pages numbered 1..67 (worksheets + games interleaved)', () => {
    expect(TOTAL_PAGES).toBe(67);
    for (let n = 1; n <= 67; n++) expect(pageByNumber(n), `page ${n}`).toBeDefined();
    expect(WORKBOOK.map((p) => p.n)).toEqual(Array.from({ length: 67 }, (_, i) => i + 1));
  });
  ...
  it('topics cover all pages exactly once', () => {
    ...
    expect(covered).toEqual(Array.from({ length: 67 }, (_, i) => i + 1));
  });
