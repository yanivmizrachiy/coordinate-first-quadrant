...
describe('workbook integrity (USER_MEMORY.md mandatory checks)', () => {
  it('has exactly 56 pages numbered 1..64 (worksheets + games interleaved)', () => {
    expect(TOTAL_PAGES).toBe(64);
    for (let n = 1; n <= 64; n++) expect(pageByNumber(n), `page ${n}`).toBeDefined();
    expect(WORKBOOK.map((p) => p.n)).toEqual(Array.from({ length: 64 }, (_, i) => i + 1));
  });
