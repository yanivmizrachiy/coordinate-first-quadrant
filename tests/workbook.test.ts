...
describe('workbook integrity (USER_MEMORY.md mandatory checks)', () => {
  it('has exactly 55 pages numbered 1..55 (worksheets + games interleaved)', () => {
    expect(TOTAL_PAGES).toBe(55);
    for (let n = 1; n <= 55; n++) expect(pageByNumber(n), `page ${n}`).toBeDefined();
    expect(WORKBOOK.map((p) => p.n)).toEqual(Array.from({ length: 55 }, (_, i) => i + 1));
  });
  ...
  it('topics cover all pages exactly once', () => {
    ...
    expect(covered).toEqual(Array.from({ length: 55 }, (_, i) => i + 1));
  });
