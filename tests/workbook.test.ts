...
describe('workbook integrity (CLAUDE.md mandatory checks)', () => {
  it('has exactly 49 pages numbered 1..49 (worksheets + games interleaved)', () => {
    expect(TOTAL_PAGES).toBe(49);
    for (let n = 1; n <= 49; n++) expect(pageByNumber(n), `page ${n}`).toBeDefined();
    expect(WORKBOOK.map((p) => p.n)).toEqual(Array.from({ length: 49 }, (_, i) => i + 1));
  });
  ...
  it('topics cover all pages exactly once', () => {
    ...
    expect(covered).toEqual(Array.from({ length: 49 }, (_, i) => i + 1));
  });
