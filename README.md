...
src/
  ...
  data/colorDecode.ts         תאי הצביעה של פענוח צבעוני — מקור אמת אחד
                              לדף המודפס ולבדיקה שמוכיחה אותו
  views/                      home (סרט הפתיחה) · menu · pageViewer · book ·
                              flipbook · coverSheet · tocSheet
...
## אין שעשועונים מקוונים

כל משימה בחוברת היא **דף מודפס** — „המשימות שלנו הן להדפסה" (31.07.2026).
שבעת השעשועונים שהיו כאן הומרו לדפי עבודה. ההוכחות עברו איתם:
`tests/printed-puzzles.test.ts` קורא את ה-HTML של כל דף מודפס ומוכיח
שהחידה שעליו פתירה, שהפתרון יחיד, ושהמילה או הקוד שמתקבלים נכונים.
יש גם בדיקת דפדפן שנכשלת על כל `button`/`select`/`textarea` בעמוד ממוספר.
