...
אפליקציה אחת ללימוד מערכת הצירים ברביע הראשון: **74 עמודים ממוספרים** (דפי עבודה ו־8 שעשועונים משולבים), עמוד שער, עמוד תוכן עניינים, סרט פתיחה, תצוגת מובייל מלאה והדפסת **A4** מקצועית — הכול בריפו אחד, בכתובת אחת ובקובץ כניסה אחד.
...
- **דפי העבודה** (`#/workbook/:n`) — 74 עמודים, ניווט עמוד־אחר־עמוד, זום, שמירת המיקום האחרון והדפסת עמוד בודד.
- **החוברת הנפתחת** (`#/book`) — חוברת דפדוף דיגיטלית: כפולה במסך רחב, דף יחיד בטלפון, גרירת דף, זום, מסך מלא ותוכן עניינים צף.
- **החוברת הרציפה** (`#/print`) — שער, עמוד תוכן עניינים צבעוני, ואחריהם כל 74 העמודים, A4 אמיתי — מכאן מדפיסים (הכול או דפים נבחרים).
- **שעשועונים** — 8 שעשועונים, כל אחד עמוד ממוספר בתוך הנושא שלו (אין אזור נפרד), שנבנו כרכיבים מתמטיים מדויקים ולא כתמונות.
